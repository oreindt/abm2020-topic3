...
knowing, avg_belief, disp = run_sim(sim, args["s"])

# write the observation to csv files
using DelimitedFiles
times = 1:args["s"]
data = [times knowing avg_belief disp]
header = ["time" "knowing" "avg_belief" "disp"]
mkpath("results")
writedlm("results/output_MH.csv", [header; data], ',')